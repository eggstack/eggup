...
| `eggup-acquisition` | Transport-neutral acquisition seam + fixture | `AcquisitionRequest`, `FetchLimits` (+ `min(request,adapter)`), `CancelFlag`, `Success \| NotFound` vs hard failure, `AcquisitionTransport`, deterministic `FixtureTransport`, exclusive 0600 temp + no-clobber promotion, URL redaction | [acquisition.md](acquisition.md) |
